...
namespace VideoIndexerArm
{
    public class Program
    {
        ...
        private const string AzureResourceManager = "management.usgovcloudapi.net";
        private const string SubscriptionId = "05a34011-9c78-465f-bad3-c62c24c534da";
        private const string ResourceGroup = "proteus-poc-rg";
        private const string AccountName = "proteusvideoindexer";
        //private const string VideoUrl = "https://demoproteusstr.blob.core.windows.net/documents/AkonDemo.mp4";
 
        private const string ApiUrl = "https://api.videoindexer.ai,azure.us";
        private const string connectionString = "DefaultEndpointsProtocol=https;AccountName=prototempstor;AccountKey=x/ET9O6s4wiY4RAq5lNDkBtkHn6wFKxWX1jhSlihpdLnqDxTWArH44oppF2LLBGkm9OCtBSb4qgQ+AStDqxM7g==;EndpointSuffix=core.usgovcloudapi.net";
        private const string storageAccount = "https://prototempstor.blob.core.usgovcloudapi.net/";
        public static async Task Main(string[] args)
        {
            List<string> videoExt = new List<string>(new string[] { ".mp4", ".avi", ".wmv", ".mov" });
            ...
            GetURIs URis = new GetURIs(connectionString);
            List<string> videoURIList = new List<string>();
            var containers = await URis.GetContainersList();
            
            Console.WriteLine("Going through loop Count: " + containers.Count);
            foreach (var item in containers)
            {
                Console.WriteLine("In foreach Loop", item);
                var uriList = URis.GetVideoURIs(item);
                foreach (var items in uriList)
                {
                    //Console.WriteLine(items);
                    if (videoExt.Any(items.Contains))
                    {
                        string videoUrl = storageAccount + items;
                        Console.WriteLine("videoUrl:  " + videoUrl);
                        videoURIList.Add(items);
                        // Upload a video
